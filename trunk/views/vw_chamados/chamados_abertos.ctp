...
array(
		'update' => 'chamados',
    	'url'=>array('controller'=>'atendimento', 'action'=>'chamadosAbertos'), 
        'indicator' => 'loadAjax',
));
...
<table cellpadding="0" cellspacing="0">
	<tr>
		...
		<th style="width: 3%;" colspan="2"><?php echo $paginator->sort('Nº', 'chamado_id');?></th>
		...
		<th style="width: 9%;><?php echo $paginator->sort('Solicitante', 'solicitante_nome');?></th>
	...
	foreach ($vw_chamados as $chamado){
		...
		$dt_abertura = date_parse($campos['chamado_abertura']);
		...
		$minutos_limite = $campos['chamado_prioridade_tempo'];
	...
	<tr <?php echo $class;?>>
		...
		<td><?php echo $campos['solicitante_nome']; ?></td>
		<td><?php
		...
		<td><?php echo $html->link('Atender', array('controller'=>'atendimento', 'action' => 'atenderChamado', $campos['chamado_id'])); ?>
	</tr>
...
<div class="barraFerramentas"><script type="text/javascript">
	jQuery(document).ready(function(){
		<?php if (!$session->check('ChamadosAbertosFiltro.filtro')){?>
...
<div class="separaElementos"></div>
	...
	Event.observe('quantidadeRegistros', 'change', function(event) { new Ajax.Updater('chamados','/helpdesk/atendimento/chamadosAbertos/limit:'+ this.value, {asynchronous:true, evalScripts:true, onComplete:function(request, json) {Element.hide('loadAjax');}, onLoading:function(request) {Element.show('loadAjax');}, requestHeaders:['X-Update', 'chamados']}) }, false);
	Event.observe('btnPesquisar', 'click', function(event) {
		if ($('valor').value != ""){ 
			new Ajax.Updater('chamados','/helpdesk/atendimento/chamadosAbertos/fil_'+$('campo').value + ":"+$('valor').value, {asynchronous:true, evalScripts:true, onComplete:function(request, json) {Element.hide('loadAjax');}, onLoading:function(request) {Element.show('loadAjax');}, requestHeaders:['X-Update', 'chamados']})
		} 
	}, false); 
	Event.observe('btnRemoverFiltro', 'click', function(event) { new Ajax.Updater('chamados','/helpdesk/atendimento/chamadosAbertos/fil_remove:true', {asynchronous:true, evalScripts:true, onComplete:function(request, json) {Element.hide('loadAjax');}, onLoading:function(request) {Element.show('loadAjax');}, requestHeaders:['X-Update', 'chamados']}) }, false);
	</script>
<div class="separaElementos"></div>
<div class="barraFerramentasElemento">
<div class="paging">
