...
array(
		'update' => 'chamados',
    	'url'=>array('controller'=>'atendimento', 'action'=>'chamadosEncerrados'), 
        'indicator' => 'loadAjax',
));
...
<table cellpadding="0" cellspacing="0">
	<tr>
		...
		<th style="width: 14%;"><?php echo $paginator->sort('Solicitante', 'solicitante_nome');?></th>
	...
	<tr <?php echo $class;?>>
		...
		<td><?php echo $html->link('Visualizar', array('controller'=>'atendimento', 'action' => 'visualizarChamado', $chamado['VwChamado']['chamado_id'], )); ?>
		</td>
...
<div class="barraFerramentas"><script type="text/javascript">
	jQuery(document).ready(function(){
		<?php if (!$session->check('ChamadosEncerradosFiltro.filtro')){?>
...
<div class="separaElementos"></div>
	...
	Event.observe('quantidadeRegistros', 'change', function(event) { new Ajax.Updater('chamados','/helpdesk/atendimento/chamadosEncerrados/limit:'+ this.value, {asynchronous:true, evalScripts:true, onComplete:function(request, json) {Element.hide('loadAjax');}, onLoading:function(request) {Element.show('loadAjax');}, requestHeaders:['X-Update', 'chamados']}) }, false);
	Event.observe('btnPesquisar', 'click', function(event) {
		if ($('valor').value != ""){ 
			new Ajax.Updater('chamados','/helpdesk/atendimento/chamadosEncerrados/fil_'+$('campo').value + ":"+$('valor').value, {asynchronous:true, evalScripts:true, onComplete:function(request, json) {Element.hide('loadAjax');}, onLoading:function(request) {Element.show('loadAjax');}, requestHeaders:['X-Update', 'chamados']})
		} 
	}, false); 
	Event.observe('btnRemoverFiltro', 'click', function(event) { new Ajax.Updater('chamados','/helpdesk/atendimento/chamadosEncerrados/fil_remove:true', {asynchronous:true, evalScripts:true, onComplete:function(request, json) {Element.hide('loadAjax');}, onLoading:function(request) {Element.show('loadAjax');}, requestHeaders:['X-Update', 'chamados']}) }, false);
	</script>
<div class="separaElementos"></div>
<div class="barraFerramentasElemento">
<div class="paging">
